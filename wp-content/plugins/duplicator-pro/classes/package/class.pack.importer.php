...
class DUP_PRO_Package_Importer
{
    ...
    {
        $tempArchiveJson = $this->extractSingleFile($relativePathInArchive, get_temp_dir());
    ...
    public function isImportable(&$failMessage = null)
    {
        ...
        if (version_compare($this->getDupVersion(), self::IMPORT_ENABLE_MIN_VERSION, '<')) {
            $failMessage = sprintf(DUP_PRO_U::__('Package version too old, %s version or higher is required.'), self::IMPORT_ENABLE_MIN_VERSION);
            return false;
        }
    ...
    private function packageHasRequiredInstallerFiles()
    {
        try {
            ...
            foreach ($requiredFilePaths as $path) {
                $tempFile = $this->extractSingleFile($path, get_temp_dir());
